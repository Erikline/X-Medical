...
from ultralytics.utils.torch_utils import (EarlyStopping, ModelEMA, de_parallel, init_seeds, one_cycle, select_device,
                                           strip_optimizer)
##### Optimizer #####
from ultralytics.engine.Optimizer.Sophia import SophiaG
...
class BaseTrainer:
    ...
    def build_optimizer(self, model, name='auto', lr=0.001, momentum=0.9, decay=1e-5, iterations=1e5):
        ...
        if name == 'Sophia':
            # Sophia 优化器需要直接传入所有参数
            optimizer = SophiaG(model.parameters(), lr=lr, betas=(momentum, 0.99), rho=0.01, weight_decay=decay)
            LOGGER.info(f"{colorstr('optimizer:')} SophiaG(lr={lr})")
            return optimizer
