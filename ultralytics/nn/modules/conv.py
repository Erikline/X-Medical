...
class CoordAttn(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        mid = 8 if 8 > inp // reduction else inp // reduction

        self.c1 = mn_conv(inp, mid, 1)

        self.conv_h = nn.Conv2d(mid, oup, 1)
        self.conv_w = nn.Conv2d(mid, oup, 1)

    def forward(self, x):
        b, c, h, w = x.size()
        x_h, x_w = self.pool_h(x), self.pool_w(x).permute(0, 1, 3, 2)

        y = self.c1(torch.cat([x_h, x_w], dim=2))
